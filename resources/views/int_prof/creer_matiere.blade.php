...
<div class="p-3 sm:ml-64 bg-no-repeat bg-cover  bg-gray-100 bg-blend-multiply">
    ...
    <h2 class="text-2xl font-semibold text-blue-600 mb-6">Créer une nouvelle matière</h2>
    ...
    <form action="{{ route('creer_matiere.store') }}" method="POST">
        ...
        <!-- Titre du cours -->
        <div class="mb-4">
            <label for="libelle_M" class="block text-lg font-medium text-blue-600">Libellé matière</label>
        ...
        <!-- Sélection du cycle -->
        <div class="mb-6">
            <label for="cycle_id" class="block text-lg font-medium text-blue-600">Cycle</label>
            <select name="cycle_id" id="cycle_id" class="mt-1 block w-full border border-gray-300 rounded px-3 py-2" required>
                <option value="">-- Sélectionnez un cycle --</option>
                @foreach($cycles as $cycle)
                    <option value="{{ $cycle->id }}">{{ $cycle->libelle_C }}</option>
                @endforeach
            </select>
            @error('cycle_id')
                <div class="text-red-600 text-sm">{{ $message }}</div>
        ...
        <div class="flex ">
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Créer le cours</button>
        </div>
    </form>
                        ...
                        @else
                            @foreach($matieres as $matiere)
                                <tr class="odd:bg-white even:bg-gray-50 border-b dark:border-gray-700 border-gray-200">
                                    ...
                                    <td class="px-6 py-4">{{ $matiere->prof->user->name }}</td>
